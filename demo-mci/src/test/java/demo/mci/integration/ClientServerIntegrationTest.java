 ...
 * 클라이언트-서버 통합 테스트
 */
@DisplayName("Client-Server Integration Tests")
...
class ClientServerIntegrationTest {
    ...
    @Test
    @Order(20)
    @DisplayName("에코 응답 확인")
    void echo_success() {
        // when
        Message response = client.echo("Hello, Integration Test!");

        // then
        assertThat(response).isNotNull();
        assertThat(response.getMessageCode()).isEqualTo(DemoMessageCodes.ECHO_RES);
        assertThat(response.getString("rspCode")).isEqualTo(DemoConstants.RSP_SUCCESS);
        assertThat(response.getString("echoData")).isEqualTo("Hello, Integration Test!");
    }

    @Test
    @Order(21)
    @DisplayName("빈 에코 데이터")
    void echo_emptyData() {
        // when
    ...
    @Test
    @Order(40)
    @DisplayName("복합 시나리오: 잔액조회 -> 이체 -> 잔액확인")
    ...
    @Test
    @Order(41)
    @DisplayName("복합 시나리오: 에코 + 하트비트 혼합")
    ...
    void mixedScenario_fullFlow() {
        ...
        // 6. 에코 확인
        assertThat(client.echo("Transaction Complete").getString("echoData")).isEqualTo("Transaction Complete");
    }
}
