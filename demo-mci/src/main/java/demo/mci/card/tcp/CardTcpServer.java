 ...
 */
@Slf4j
public class CardTcpServer {

    private final TcpServer server;
    private final BizRegistry bizRegistry;

    public CardTcpServer(int port) {
        // 레이아웃 등록
    ...
    private Message createErrorResponse(Message request) {
        ...
        String msgCode = request.getMessageCode();
        response.setField("msgCode", msgCode != null && msgCode.length() >= 3 ? msgCode.substring(0, 3) + "2" : "ERR2");
    ...
    public void start() {
        server.start();
        log.info("Card TCP Server started on port {}", server.getConfig().getPort());
    }
    ...
    public void stop() {
        server.stop();
        log.info("Card TCP Server stopped");
    }
    ...
    public static void main(String[] args) {
        int port = args.length > 0 ? Integer.parseInt(args[0]) : DemoConstants.DEFAULT_CARD_TCP_PORT;

        CardTcpServer cardServer = new CardTcpServer(port);
        cardServer.start();
